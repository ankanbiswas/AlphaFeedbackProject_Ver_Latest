...
%     FS = [4,8,7,5,8,3,9,10,10,7,10,8,10,10,9,10,9,9,9, 9,10,10,8]; 
    ...
    AllSub_pVal_sl_ConstantTrials   = [];
    AllSub_pVal_sl_ValidTrials      = [];
    AllSub_pVal_sl_InvalidTrials    = [];
    AllSub_mean_ValidTrials =[];
    AllSub_mean_InvalidTrials =[];
    ...
    % slope analysis for constant trials, all subject:
    for i = 1: numSubjects
        subNum = i;
        ...
        temp_pVal_sl                    = lm.Coefficients{2,4};
        AllSub_pVal_sl_ConstantTrials    = [AllSub_pVal_sl_ConstantTrials;temp_pVal_sl];
    end
    
    %----------------------Addition---------on 27th august
    % slope analysis for Invalid trials, all subject:
    
     for i = 1: numSubjects
        subNum = i;
        meanAlphaPowerInvalid  = EX_mean_deltaPowerVsTimeList_invalid(subNum,:);
        lm = fitlm(x_axis,meanAlphaPowerInvalid,'linear') ;
        %         temp_sl_coefficients = polyfit(x_axis,meanAlphaPowerConstant,1);
        
        x = x_axis(:);
        X = [ones(size(x)) x];
        y = meanAlphaPowerInvalid(:);
        [b,~,~,~,stats] = regress(y,X);
        p_value_regress = stats(3);
        slope_value_regress = b(2);
        temp_mean = mean(y);
        AllSub_mean_InvalidTrials = [AllSub_mean_InvalidTrials;temp_mean];
        temp_Slope                      = slope_value_regress;
        AllSub_sl_InvalidTrials         = [AllSub_sl_InvalidTrials;temp_Slope];
        temp_pVal_sl                    = p_value_regress;
        AllSub_pVal_sl_InvalidTrials    = [AllSub_pVal_sl_InvalidTrials;temp_pVal_sl];
     end
    
     % slope analysis for Valid trials, all subject:
      for i = 1: numSubjects
        subNum = i;
        meanAlphaPowerValid      = EX_mean_deltaPowerVsTimeList_valid(subNum,:);
        lm = fitlm(x_axis,meanAlphaPowerValid,'linear') ;
        %         temp_sl_coefficients = polyfit(x_axis,meanAlphaPowerConstant,1);
        x = x_axis(:);
        X = [ones(size(x)) x];
        y = meanAlphaPowerValid(:);
        [b,~,~,~,stats] = regress(y,X,0.01);
        p_value_regress = stats(3);
        slope_value_regress = b(2);
        
        temp_mean = mean(y);
        AllSub_mean_ValidTrials = [AllSub_mean_ValidTrials;temp_mean];
        temp_Slope                  = slope_value_regress;
        AllSub_sl_ValidTrials       = [AllSub_sl_ValidTrials;temp_Slope];
        temp_pVal_sl                = p_value_regress;
        AllSub_pVal_sl_ValidTrials  = [AllSub_pVal_sl_ValidTrials;temp_pVal_sl];
      end
