...
function [PlotH,colorNames,meanEyeOpenPowerList,...
    ...
    colorNames{1}      = rgb('Lime');
    colorNames{2}      = rgb('DeepSkyBlue');
    colorNames{3}      = rgb('DimGray');
...
%             h=[];
            for i=1:1:4 % Trial Type:  Valid: 1, Invalid: 2, Constant: 3;
...
%                    h(i)= plot(plotH_deltaPowerVsTime,analysisData.timeValsTF,mean(deltaPowerVsTimeList(trialPos,:),1),'color',colorNames(i));
                    ...
                    switch i
                        case 1
                            ...
                            if(length(trialPos)==7 || length(trialPos)==6 || length(trialPos)==5 )
                        ...
                        case 3
                            ...
                            text(0.6,0.9,strcat(num2str(length(trialPos)),';'),'Color',[colorNames{i}],'fontsize',fontSizeVal,'fontweight','bold','unit','normalized','parent',plotH_deltaPowerVsTime);
                            %                     otherwise
                        case 4
                            validTrialPos = [First_catTrialPos,Second_catTrialPos,Third_catTrialPos];
                            sortValidTrialPos = sort(validTrialPos); 
                            plot(plotH_deltaPowerVsTime,analysisData.timeValsTF,mean(deltaPowerVsTimeList(sortValidTrialPos,:),1),'color','r');
                            
                            deltaPowerVsTimeList_ValidAll = deltaPowerVsTimeList(sortValidTrialPos,:);
                            mean_deltaPowerVsTimeList_ValidAll = mean(deltaPowerVsTimeList_ValidAll,1);
                            EX_mean_deltaPowerVsTimeList_ValidAll = mean_deltaPowerVsTimeList_ValidAll(timePosAnalysis);
                            if subInd == 6  
                                text(0.8,0.27,num2str(mean(EX_mean_deltaPowerVsTimeList_ValidAll),'%.2f'),'Color','r','fontsize',fontSizeVal,'fontweight','bold','unit','normalized','parent',plotH_deltaPowerVsTime);
                            elseif subInd == 9 || subInd ==17 
                                text(0.8,0.6,num2str(mean(EX_mean_deltaPowerVsTimeList_ValidAll),'%.2f'),'Color','r','fontsize',fontSizeVal,'fontweight','bold','unit','normalized','parent',plotH_deltaPowerVsTime);
                            else
                                text(0.8,0.25,num2str(mean(EX_mean_deltaPowerVsTimeList_ValidAll),'%.2f'),'Color','r','fontsize',fontSizeVal,'fontweight','bold','unit','normalized','parent',plotH_deltaPowerVsTime);
                            end
                            
                            text(0.7,0.9,num2str(length(sortValidTrialPos)),'Color','r','fontsize',fontSizeVal,'fontweight','bold','unit','normalized','parent',plotH_deltaPowerVsTime);   
